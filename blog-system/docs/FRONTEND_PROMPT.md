...
- **数据库**：`dev` 使用 H2 内存库（`jdbc:h2:mem:blogdb`），`prod` 使用 PostgreSQL
- **端口**：8080
...
| `/login` | GET | 登录页面 | ❌ 否 |
| `/login` | POST | 提交登录 | ❌ 否 |
| `/logout` | POST | 退出登录 | ✅ 是（需 CSRF） |
| `/admin` | GET | 管理页面，显示文章列表 | ✅ 是 |
| `/post` | POST | 发布新文章 | ✅ 是 |
| `/post/delete/{id}` | POST | 删除文章 | ✅ 是（需 CSRF） |

### 3. 登录信息

- **用户名**：`admin`
- **密码**：`123456`
- 登录态由 Spring Security 维护（Session Cookie）
- 已启用 CSRF：Thymeleaf 的 `<form th:action=... method="post">` 会自动注入 CSRF hidden input

### 4. 拦截器

不再使用自定义拦截器；访问控制由 Spring Security 统一处理（未登录访问受保护路径会重定向到 `/login`）。
...
3. **表单**：`<form method="post" th:action="...">`（Thymeleaf 会自动注入 CSRF hidden input）
...
│   │   └── config/
│   │       ├── AdminUserProperties.java  # 管理员账号配置
...
- 后端已接入 Spring Security（含 CSRF），请尽量使用 `th:action` 的表单写法
- 所有页面都有对应的 Controller 方法，model 中有需要的数据
- 未登录访问管理页面会被重定向到 `/login`
- `dev` 默认使用 H2 内存库；`prod` 使用 PostgreSQL
